 ...
 * This class is used as a helper class for rendering on SurfaceViews and
 * TextureViews.
 */
public class EglRenderer {
  ...
  private WeakReference<ITextureFrameHelper> textureFrameHelper;
  ...
  public void init(final EglBase.Context sharedContext, final int[] configAttributes,
    ...
    synchronized (handlerLock) {
      ...
      synchronized(helperLock) {
        textureFrameHelper = new WeakReference<>(helper);
      }
    }
  }
  ...
  /**
   * Registers a callback to invoke when a new video frame is received. This version uses
   * the drawer of the EglRenderer passed in init.
   *
   * @param listener The callback to invoke. The callback will be invoked on the render thread.
  ...
  public void onFrame(VideoFrame frame) {
    ...
    synchronized (handlerLock) {
      if (renderThreadHandler == null) {
        ...
        frame.release();
        synchronized(helperLock) {
          if(textureFrameHelper != null && textureFrameHelper.get() != null) {
            textureFrameHelper.get().returnTexture();
          }
        }
        return;
      }
...
//      long startMs = System.currentTimeMillis();
     ...
     //texture draw cost 2-2
//      long endMs = System.currentTimeMillis();
//      long costMs = endMs - startMs;
//      logD("::texture draw costMs="+costMs);
